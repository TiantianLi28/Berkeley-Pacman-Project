...
def cornersHeuristic(state, problem):
    ...
    total=0
    for c in corner:
        dis=util.manhattanDistance(position, c)
        if dis>total:
            total=dis
    #total+=(len(corner)-1)
    return total
...
def foodHeuristic(state, problem):
  ...
  farPoint1=foodList[0]
  farPoint=foodList[-1]

  cost=mazeDistance(farPoint1, farPoint, problem.startingGameState)
  cost+=min(mazeDistance(position, farPoint, problem.startingGameState), mazeDistance(farPoint1, position, problem.startingGameState))
  return cost
