        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for pos in self.legalPositions:
                allPossible[pos] += newPosDist[pos]*self.beliefs[p];
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while(numP<self.numParticles):
            for pos in self.legalPositions:
                partic.append(pos)
                numP+=1
        self.particle=partic
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            while(n<self.numParticles):
                par.append(self.getJailPosition())
                n+=1
            self.particle=par
        else:
            belief=util.Counter()
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particle:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            #update according to distribution
            par.append(util.sample(newPosDist))
        self.particle=par
